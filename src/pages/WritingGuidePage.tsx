...
export default function WritingGuidePage() {
  ...
  useEffect(() => {
    const checkProgress = async () => {
      if (!user) return;
      const progress = await getModuleProgress(user.id, 'Writing Guide');
  ...
  const handleComplete = async () => {
    ...
    setIsCompleted(true);
    try {
      await markModuleComplete(user.id, 'Writing Guide');
      toast({
        title: 'Completed',
        description: 'Writing Guide has been marked as completed.',
      });
    } catch (error) {
